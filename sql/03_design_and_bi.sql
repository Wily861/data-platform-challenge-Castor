...
-- Pre-agregación de métricas mensuales para Power BI[cite: 22].
...
-- Índice único para permitir REFRESH CONCURRENTLY sin bloquear Power BI[cite: 16, 22].
CREATE UNIQUE INDEX idx_mv_monthly_sales_unique ON mv_bi_monthly_sales (product_id, month);

-- Estrategia de Storage (Punto 2.2)
-- Separación física de datos para optimizar I/O[cite: 23].
